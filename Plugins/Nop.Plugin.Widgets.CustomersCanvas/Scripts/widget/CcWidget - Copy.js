var CcWidget = (function () {
    ...
    CcWidget.prototype.replaceCartItemImage = function (id, index, imageSource) {
        var input = $("input[name=\"removefromcart\"][value=\"" + id + "\"]");
        var row;
        var img = null;
        if (input.length > 0) {
            row = input.parent().parent();
            img = $("td:nth-child(2) a img", row);
        }
        else if (index > -1) {
    ...
    CcWidget.prototype.replaceReturnToEditUrl = function (id, oldUrl, url) {
        var input = $("input[name=\"removefromcart\"][value=\"" + id + "\"]");
        var row;
        var link = null;
        link = $('a[href="' + oldUrl + '"]');
        if (link.length < 1) {
            if (input.length > 0) {
                row = input.parent().parent();
                link = $("td:nth-child(3) .edit-item a", row);
                if (link.length < 1) {
