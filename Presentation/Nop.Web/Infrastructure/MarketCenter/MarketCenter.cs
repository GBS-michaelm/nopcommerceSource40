...
{
    public class MarketCenterWeb : CategoryModel
    {
        ...
        List<MarketCenterWeb> _childCompanies = new List<MarketCenterWeb>();
        ...
        public MarketCenterWeb(int marketCenterCategoryId, bool lightVer = true, bool getChildren = false, string customType = "") {
        ...
        public List<MarketCenterWeb> childCompanies { get { return _childCompanies; } set { _childCompanies = value; } }
        ...
        private int GetChildCategories(int marketCenterCategoryId, string type, bool countOnly = false) {
            ...
            if (!countOnly)
            {
                if (marketCenterChildCompanyDataView.Count > 0)
                {
                    ...
                    {
                        MarketCenterWeb marketCenter = new MarketCenterWeb(Int32.Parse(marketCenterChildCompanyDataView[i]["id"].ToString()), customType: type);
        ...
        public Dictionary<string, string> GetMarketCenterHtml(string type, bool hack) {
            ...
            List<MarketCenterWeb> featuredMarketCenterList = new List<MarketCenterWeb>();
            List<MarketCenterWeb> alphaList1 = new List<MarketCenterWeb>();
            List<MarketCenterWeb> alphaList2 = new List<MarketCenterWeb>();
            List<MarketCenterWeb> alphaList3 = new List<MarketCenterWeb>();
            ...
            try
            {
                if (!hack)
                {
                    ...
                    //foreach (var marketcenterCategory in TopLevelMarketCenterIdsInOrder)
                    {
                        ...
                        //else
                        //{
                        marketcenter = new MarketCenterWeb(TopLevelMarketCenterIdsInOrder[i].Id, getChildren: true);
        ...
        private List<MarketCenterWeb> GetTabData(bool? isFeatured = null, char start = '!', char end = 'z', int specAttrOpId = 0, string type = "") {

            List<MarketCenterWeb> marketCenters = null;
            ...
            try
            {
                if (string.IsNullOrEmpty(type))
                {
                    ...
                    marketCenters = JsonConvert.DeserializeObject<List<MarketCenterWeb>>(jsonResult);
                }
                else
                {
        ...
        private string BuildTabHtml(List<MarketCenterWeb> marketcenterList, string type) {
        ...
        private string BuildInnerTabLink(MarketCenterWeb marketcenter, string type) {
        ...
        private string BuildFeaturedCompanyHtml(List<MarketCenterWeb> marketcenterList, string type) {
        ...
        private string BuildChildCompanyHtml(List<MarketCenterWeb> marketcenterList, string type) {
